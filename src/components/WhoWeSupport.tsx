...
export default function WhoWeSupport() {
  ...
  return (
    <section className="py-24 sm:py-32 bg-brand-50/50 relative overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
        ...
        {/* Condition pills */}
        <div ref={pillsRef} className="flex flex-wrap justify-center gap-3 mb-16">
          {conditions.map((condition, i) => (
            <motion.span
              ...
              className="inline-block bg-white border border-brand/20 text-brand font-medium px-5 py-2.5 rounded-full text-sm shadow-sm"
            >
              {condition}
            </motion.span>
          ))}
        ...
        <div ref={fundingRef} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 max-w-4xl mx-auto">
          {fundingSources.map((source, i) => (
            ...
            >
              ...
              <p className="text-gray-500 text-xs mt-1">{source.desc}</p>
            </motion.div>
          ))}
        </div>
      </div>
